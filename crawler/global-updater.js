...
const GlobalCrawler = require('./global-crwaler');
...
async function crawlAndUpdateGlobal(outputPath, apiClient) {
    ...
    console.log('gloablStat updated successfully');
}

function findUpdatedRows(newRowByCc, oldRowByCc) {
    const updatedRows= [];
    for (const cc of Object.keys(newRowByCc)) {
        const newRow = newRowByCc[cc];
        const oldRow = oldRowByCc[cc];
